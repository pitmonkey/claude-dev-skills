...
Decide the final label set (the chosen content label, plus the `claude-drafted` marker).
...
- The `claude-drafted` marker needs no pre-check: include it in the label set and it is
  auto-created at issue time if missing.
...
- **remote (MCP):** call `mcp__github__issue_write` with:
  ...
  - `labels`: the final set — the chosen content label (if kept) plus `"claude-drafted"`;
    omit the content label if the user skipped it, but **always keep `"claude-drafted"`**
  - `body`: the drafted body (omit the `## ⚠️ Suggested Fix` section entirely if no fix was
    identified — do not invent one):
...
Print the created issue's URL, then note the marker:
